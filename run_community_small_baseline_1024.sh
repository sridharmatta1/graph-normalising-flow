#!/bin/bash
#SBATCH --job-name=GNF_community_baseline
#SBATCH --output=logs/community_baseline_output.log
#SBATCH --error=logs/community_baseline_error.log
...
# ============================================================
# Baseline (unconditioned) GNF on community-small -- single seed,
# not a 1-5 array. One full run already produces the target 1024
# generated graphs, matching the ego-small baseline's single-seed
# convention (run_ego_small_baseline_1024.sh).
# ============================================================
SEED=1
PYTHON=/home/bidaralli/miniconda3/envs/SRP_Graphs/bin/python
WORKDIR=/home/bidaralli/Graph-Normalising-flow
RESULTS_DIR=$WORKDIR/results/community_1024/seed_$SEED

# ============================================================
# HYPERPARAMETERS  (community-small paper settings -- matches
# bidaralli's baseline run.sh exactly)
# ============================================================
...
srun $PYTHON -u $WORKDIR/run_gnn.py \
    ...
    --wandb_run_name "community_baseline_seed${SEED}_gnn"
...
srun $PYTHON -u $WORKDIR/train_grevnet_with_data.py \
    --dataset $DATASET \
    ...
    --max_checkpoints_to_keep 5 \
    --wandb_project $WANDB_PROJECT \
    --wandb_run_name "community_baseline_seed${SEED}_grevnet"
...
srun $PYTHON -u $WORKDIR/generate_graphs.py \
    ...
    --wandb_project $WANDB_PROJECT \
    --wandb_run_name "community_baseline_seed${SEED}_generate"
...
srun $PYTHON -u $WORKDIR/compute_mmd.py \
    ...
    --wandb_project $WANDB_PROJECT \
    --wandb_run_name "community_baseline_seed${SEED}_mmd"
